...
(cd $user_snapshots/gcc.git; git fetch '+refs/changes/*:refs/remotes/gerrit/changes/*')
...
if test x"${runtests}" = xtrue -a x"${logserver}" != x""; then
...
# machine.
if test x"${canadian}" = x"true" -a x"${logserver}" != x""; then
...
if test x"${sums}" != x -o x"${runtests}" != x"true" -a x"${logserver}" != x""; then
    if test x"${sums}" != x; then
	test_logs=""
	for s in ${sums}; do

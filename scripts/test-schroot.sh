...
if $ssh_master; then
    ssh -o ControlMaster=yes -o ControlPersist=5m -o ControlPath=/tmp/ssh-%u-%r@%h:%p $ssh_opts -fN $target
    ssh -v -o ControlMaster=no -o ControlPersist=5m -o ControlPath=/tmp/ssh-%u-%r@%h:%p $ssh_opts $target echo "Can ssh using master connection if the above log is short"
fi

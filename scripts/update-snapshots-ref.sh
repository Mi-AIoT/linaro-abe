...
generate_snapshots ()
{
    ...
    git reset --hard
    git clean -fd
    ./configure --with-local-snapshots=$snapshots_dir --with-fileserver=ex40-01.tcwglab.linaro.org/snapshots-ref
...
for repo in $snapshots_dir/*.git; do
    (
	cd $repo
	git remote update -p
...
if true; then
	...
	(
	    rsync -az --delete $snapshots_dir/ $M:$snapshots_dir-new/

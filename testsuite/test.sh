...
if test $? -eq 0 -a -e ${local_snapshots}/gmp-6.1.2.tar.xz; then
    pass "fetch_http gmp"
else
    fail "fetch_http gmp"
fi
...
gmp_stamp1=`stat -c %X ${local_snapshots}/gmp-6.1.2.tar.xz`
...
gmp_stamp2=`stat -c %X ${local_snapshots}/gmp-6.1.2.tar.xz`
...
# Get the timestamp of the file after another fetch.
gmp_stamp3=`stat -c %X ${local_snapshots}/gmp-6.1.2.tar.xz`
...
rm ${local_snapshots}/gmp-6.1.2.tar.xz
...
if test $? -gt 0 -o ! -e "${local_snapshots}/gmp-6.1.2.tar.xz"; then
...
rm -f ${local_refdir}/gmp-6.1.2.tar.xz
echo "DEADBEEF" > ${local_refdir}/gmp-6.1.2.tar.xz
...
if test $? -gt 0; then
    fail "fetch gmp (with full name) from reference dir failed unexpectedly."
elif test x"$(grep DEADBEEF ${local_snapshots}/gmp-6.1.2.tar.xz)" != x""; then
...
rm ${local_snapshots}/gmp-6.1.2.tar.xz
rm ${local_refdir}/gmp-6.1.2.tar.xz
...
# a download
rm ${local_snapshots}/gmp-6.1.2.tar.xz &>/dev/null
rm ${local_refdir}/gmp-6.1.2.tar.xz &>/dev/null
...
cp ${local_snapshots}/gmp-6.1.2.tar.xz ${local_refdir}/ &>/dev/null
...
mv ${local_snapshots}/gmp-6.1.2.tar.xz ${local_snapshots}/gmp-6.1.2.tar.xz.back
echo "empty file" > ${local_snapshots}/gmp-6.1.2.tar.xz
...
mv ${local_snapshots}/gmp-6.1.2.tar.xz.back ${local_snapshots}/gmp-6.1.2.tar.xz

echo "============= extract() tests ================"

# use correct md5sum
set_component_md5sum gmp f58fa8001d60c4c77595fbbb62b63c1d

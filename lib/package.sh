...
#    trace "$*"
	...
	if test x"${component}" = x"abe"; then
	    ...
	    local configure="`get_component_configure ${component} | sed -e "s:${local_builds}:\$\{local_builds\}:g" -e "s:${sysroots}:\$\{sysroots\}:g" -e "s:${snapshots}:\$\{snapshots\}:g" -e "s:${host}:\$\{host\}:g"`"
	...
	local makeflags="`get_component_makeflags ${component} | sed -e "s:${local_builds}:\$\{local_builds\}:g" -e "s:${sysroots}:\$\{sysroots\}:g" -e "s:${snapshots}:\$\{snapshots\}:g" -e "s:${host}:\$\{host\}:g"`"
	...
	if test x"${component}" = x"gcc"; then
	    local stage1="`get_component_configure gcc stage1 | sed -e "s:${local_builds}:\$\{local_builds\}:g" -e "s:${sysroots}:\$\{sysroots\}:g" -e "s:${snapshots}:\$\{snapshots\}:g" -e "s:${host}:\$\{host\}:g"`"
	    echo "gcc_stage1_flags=\"${stage1}\"" >> ${outfile}
	    local stage2="`get_component_configure gcc stage2 | sed -e "s:${local_builds}:\$\{local_builds\}:g" -e "s:${sysroots}:\$\{sysroots\}:g" -e "s:${snapshots}:\$\{snapshots\}:g" -e "s:${host}:\$\{host\}:g"`"

...
read_conf_files ()
{
	...
	#    aarch64_errata languages tag
        ...
        #    preferred_libc
        for var in default_configure_flags default_makeflags latest mingw_only runtest_flags stage1_flags stage2_flags static_link; do
            if [ "${!var:+set}" = "set" ]; then
...
#    trace "$*"
    ...
    local current_component_version="${!versionvar}"
    local use_version=

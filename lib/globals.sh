...
#    trace "$*"
	...
	for i in ${components}; do
		...
		*)
            ...
            # for old manifests, we try to behave like the old version
            # of Abe.
            if $fixup_mingw; then
                mingw_only=no
            fi

	    component_init $i ${branch:+BRANCH=${branch}} ${revision:+REVISION=${revision}} ${url:+URL=${url}} ${filespec:+FILESPEC=${filespec}} ${srcdir:+SRCDIR=${srcdir}} ${builddir:+BUILDDIR=${builddir}} ${stage1_flags:+STAGE1=\"${stage1_flags}\"} ${stage2_flags:+STAGE2=\"${stage2_flags}\"} ${configure:+CONFIGURE=\"${configure}\"} ${makeflags:+MAKEFLAGS=\"${makeflags}\"} ${static:+STATICLINK=${static}} ${md5sum:+MD5SUM=${md5sum}} ${mingw_only:+MINGWONLY=${mingw_only}}
	    if [ $? -ne 0 ]; then
...
get_component_list()
{
	...
	if is_host_mingw; then
	    ...
	    # don't need to rebuild the sysroot.
            builds="${builds} binutils libc stage2 gdb"
	else

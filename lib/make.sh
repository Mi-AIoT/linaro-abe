...
build()
{
	...
	# TODO Why is this check here? If we're not running the tests, and the
	# stamp is current/update to date we just exit. If we are running test
	# and the stamp is up to date, presumably we reconfigure/rebuild?
        if test x"${runtests}" = x; then
            return 0 
        fi
